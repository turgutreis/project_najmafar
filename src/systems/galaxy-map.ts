...
export function renderGalaxyMap() {
    ...
    drawGalaxyMap = function(mouseX = -1, mouseY = -1) {
        ...
        // 2. Concentric Galactic Sectors (Core -> Mid-Rim -> Outer-Rim)
        // Sector 3: Core (Radius <= 110)
        ...
        // Sector 2: Mid-Rim (Radius 110 - 265)
        ...
        // 3. Current Player System Halos & Warp / Psionic Range
        ...
        // Pre-calculate Hover System: Find the closest star to the cursor within hit radius
        let minHoverDist = 14;
        hoverSystem = null;
    ...
    canvas.onwheel = (e) => {
        ...
        const newZoom = Math.min(6.0, Math.max(0.5, mapZoom * zoomFactor));
        mapZoom = newZoom;
    };
    ...
    canvas.onclick = (e) => {
        ...
        if (dragDist > 5) {
            return; // Ignore click if user was panning the map
        }
